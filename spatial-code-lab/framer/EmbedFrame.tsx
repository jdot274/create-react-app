...
export default function EmbedFrame({
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) { setShouldLoad(true); obs.disconnect() }
    }, { threshold: 0.1 })
    ...
    <div ref={containerRef} style={{
      ...
      {showChrome && (
        <div style={{
          ...
          <div style={{
            flex: 1, margin: "0 16px", padding: "4px 12px", borderRadius: 8,
            background: "rgba(255,255,255,0.04)",
            border: "1px solid rgba(255,255,255,0.06)",
            fontSize: 11, color: "rgba(248,250,252,0.3)",
            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          }}>{src || `${embedType}.example.com/…`}</div>
          <span style={{
      ...
      <div style={{ position: "relative", height }}>
        {src && shouldLoad ? (
          ...
          <div style={{
            ...
            <p style={{
              fontSize: 12, color: "rgba(248,250,252,0.28)",
              textAlign: "center", maxWidth: 260, lineHeight: 1.6,
            }}>{PLACEHOLDER_LABELS[embedType]}</p>
          </div>
        )}
      </div>
    </div>
...
addPropertyControls(EmbedFrame, {
  src: {
    type: ControlType.String, title: "Embed URL", defaultValue: "",
    description: "The URL to embed (Spline, CodePen, Figma, etc.)",
  },
  title: { type: ControlType.String, title: "Title", defaultValue: "Embed" },
  embedType: {
    type: ControlType.Enum, title: "Type", defaultValue: "custom",
    options: ["spline", "codepen", "figma", "penpot", "custom"],
    optionTitles: ["Spline 3D", "CodePen", "Figma", "Penpot", "Custom"],
  },
  height: { type: ControlType.Number, title: "Height", defaultValue: 400, min: 160, max: 800, step: 20 },
  cornerRadius: { type: ControlType.Number, title: "Radius", defaultValue: 16, min: 0, max: 32, step: 2 },
  showChrome: { type: ControlType.Boolean, title: "Show Chrome", defaultValue: true },
  lazyLoad: { type: ControlType.Boolean, title: "Lazy Load", defaultValue: true },
})
