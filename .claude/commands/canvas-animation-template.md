...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  body{background:#111;display:flex;flex-direction:column;align-items:center;
       justify-content:center;min-height:100vh;gap:16px;font-family:system-ui}
  canvas{display:block;border-radius:12px}
  .btns{display:flex;gap:10px}
  button{background:#222;color:#fff;border:1px solid #333;padding:9px 18px;
...
GIFEncoder.prototype.encode=function(){
  ...
  o.push(0x21,0xFF,0x0B);for(const c of [78,69,84,83,67,65,80,69,50,46,48])o.push(c);o.push(3,1,0,0,0);
  for(const f of this.frames){
    const{pixels:px,palette:pal}=quant(f.d,this.w,this.h);const cb=pb(pal.length);
    o.push(0x21,0xF9,4,0);p16(o,Math.round(f.t/10));o.push(0,0);
    o.push(0x2C);p16(o,0);p16(o,0);p16(o,this.w);p16(o,this.h);o.push(0x80|(cb-1));
...
function quant(id,w,h){
  ...
  for(let i=0;i<w*h;i++){
    ...
    const qr=(r>>3)<<3,qg=(gv>>3)<<3,qb=(b>>3)<<3,key=(qr<<16)|(qg<<8)|qb;
    if(!m.has(key)){
      ...
      m.set(key,pal.length);pal.push([qr,qg,qb]);
    }
    idx[i]=m.get(key);
  }
...
function enc(px,mb){
  ...
  for(let i=1;i<px.length;i++){
    ...
    if(t.has(sc)){s=sc;}else{emt(t.get(s));if(nc<=4095){t.set(sc,nc++);if(nc>(1<<cs)&&cs<12)cs++;}else{emt(cl);init();nc=ei+1;cs=mb+1;}s=ch;}
  }
...
document.getElementById('gif-btn').onclick = async function() {
  ...
  const FPS=20, DUR=4, FRAMES=FPS*DUR, delay=Math.round(1000/FPS);
  const enc = new GIFEncoder(W, H);
  myAngle = 0; lastT = 0; // reset state
  ...
  btn.textContent='Building…'; await new Promise(r=>setTimeout(r,0));
  dl(new Blob([enc.encode()],{type:'image/gif'}),'output.gif');
  btn.disabled=false; btn.textContent='Save .gif';
  t0=null; myAngle=0; lastT=0;
};
...
document.getElementById('mp4-btn').onclick = function() {
  ...
  t0=null; myAngle=0; lastT=0; // restart clean
  ...
  mr.onstop=()=>{
    ...
    btn.disabled=false; btn.textContent='Save .mp4';
  };
  mr.start(100); setTimeout(()=>mr.stop(),4000);
};
...
```js
function draw(t) { ctx.drawImage(videoEl, 0, 0, W, H); }
```
